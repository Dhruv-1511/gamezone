      ...
      try {
        ...
        if (!Array.isArray(games) || games.length === 0) {
          ...
          if (saveRes.status === 401) {
            addLog(`  ✗ ${cat.name}: Unauthorized — check the API key above`)
            continue
          }
  ...
  return (
    <>
      ...
      <div style={{
        ...
        <input
          type="password"
          placeholder="Backend API_KEY (from your .env)"
          value={apiKey}
        ...
        <button
          ...
          style={{
            background: status === 'running' || !apiKey ? '#444' : '#e94560',
            color: 'white',
            border: 'none',
            padding: '14px 32px',
            borderRadius: '10px',
            fontSize: '16px',
            fontWeight: '700',
            cursor: status === 'running' || !apiKey ? 'not-allowed' : 'pointer',
            marginBottom: '24px',
            width: '100%',
          }}
        >
          {status === 'idle' && (apiKey ? '🚀 Populate Database' : '🔑 Enter API key first')}
        ...
        {logs.length > 0 && (
          <div style={{
            background: '#1a1a2e', border: '1px solid #2d2d4e', borderRadius: '10px', padding: '16px',
